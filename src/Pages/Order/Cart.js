...
const Cart = () => {
  ...
  if (user) {
    fetch(`https://e-sheba-server.herokuapp.com/userInfo/${user.email}`)
      .then((res) => res.json())
  ...
  const handelDelete = () => {
    fetch(`https://e-sheba-server.herokuapp.com/deleteFromCart/${deleteItem}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handelCartForm = (e) => {
    ...
    if (user) {
      ...
      if (email !== null) {
        fetch(`https://e-sheba-server.herokuapp.com/newUser/${email}`, {
          method: "PUT",
          headers: {
